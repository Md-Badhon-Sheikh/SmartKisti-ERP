<x-app-layout :title="__('New Manufacturer')">
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 pt-6">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('New Manufacturer') }}</h3>
            </div>
        </div>
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8 fv-row">
                        <select name="type" class="form-select form-select-solid form-select-lg" required>
                            ...
                            <option value="own_factory" @selected(old('type') === 'own_factory')>{{ __('Own Factory') }}</option>
                            <option value="local_carpenter" @selected(old('type') === 'local_carpenter')>{{ __('Local Carpenter') }}</option>
                            <option value="outside_factory" @selected(old('type') === 'outside_factory')>{{ __('Outside Factory') }}</option>
                        </select>
                        @error('type')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
